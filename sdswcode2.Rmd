...
alldata$Percentile <- round(((441-alldata_CtoP_sorted$rank)*100)/440, digits = 0)
alldata <- alldata[order(alldata$Restaurant),]

# Mean_CtoP <- mean(alldata$Calories_to_Price)
# SD_CtoP <- sqrt(var(alldata$Calories_to_Price))
# alldata$Percentile <- round(100*(1 - pnorm(alldata$Calories_to_Price, mean = Mean_CtoP, sd = SD_CtoP)), digits = 0)
```

```{r}
CtoPdata <- alldata %>%
  dplyr::select(Restaurant, Item, Price, Calories, Calories_to_Price, Percentile)
...
ggplot(data=arrange(alldata, Calories_to_Price)[1:10,], aes(x=Calories_to_Price, y=reorder(Item, -Calories_to_Price), fill=Restaurant)) + 
...
ggplot(alldata_avg, aes(x=AvgCarbohydrates_to_Calories, y=reorder(Restaurant, AvgCarbohydrates_to_Calories))) + 
  geom_bar(stat='identity') + 
  labs(x='Carbohydrates (g)', y='Restaurants') +
  geom_bar(position = 'dodge', stat='identity') +
...
ggplot(alldata_avg, aes(x=AvgSodium_to_Calories, y=reorder(Restaurant, -AvgSodium_to_Calories))) + 
  geom_bar(stat='identity') + 
  labs(x='Sodium (mg)', y='Restaurants') +
  geom_bar(position = 'dodge', stat='identity') +
...
ggplot(alldata_avg, aes(x=AvgSaturatedFat_to_Calories, y=reorder(Restaurant, -AvgSaturatedFat_to_Calories))) + 
  geom_bar(stat='identity') + 
  labs(x='Saturated Fat (g)', y='Restaurants') +
  geom_bar(position = 'dodge', stat='identity') +
...
ggplot(alldata_avg, aes(x=AvgDietaryFiber_to_Calories, y=reorder(Restaurant, AvgDietaryFiber_to_Calories))) + 
  geom_bar(stat='identity') + 
  labs(x='Dietary Fiber (g)', y='Restaurants') +
  geom_bar(position = 'dodge', stat='identity') +
