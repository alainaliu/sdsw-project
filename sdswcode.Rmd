...
CarlsJr_Mean_CtoP <- mean((na.omit(alldata[alldata$Restaurant == "Carl's Jr.",]))$Calories_to_Price)
...
CarlsJr_Mean_Protein <- mean((na.omit(alldata[alldata$Restaurant == "Carl's Jr.",]))$Protein)
...
```{r}
#Sorting Calories to Price for each resturant so we can see which item is the best deal for each
